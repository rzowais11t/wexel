...
// Must be at module level — creating this inside a render function
// causes a new component class to be registered every frame, breaking EffectComposer.
...
function DitheredWaves({
  ...
  useFrame((_state, delta) => {
    ...
    if (!disableAnimation) {
      ...
      if (u.time) u.time.value = timeAccRef.current;
    }
    ...
    if (!prevColor.current.every((v, i) => v === waveColor[i])) {
      if (u.waveColor) (u.waveColor.value as THREE.Color).set(...waveColor);
    ...
    if (enableMouseInteraction) {
      if (u.mousePos) (u.mousePos.value as THREE.Vector2).copy(mouseRef.current);
    }
  });
    ...
    <>
      <mesh ref={mesh} scale={[viewport.width, viewport.height, 1]}>
        ...
        <shaderMaterial
          ...
          uniforms={waveUniformsRef.current as unknown as { [key: string]: THREE.IUniform }}
        />
      </mesh>
